...
class Update extends UserModel
{
    ...
    public function rules()
    {
        return $this->mergeRules([
            ...
            [['id'], 'customer', 'callback' => function ($value) {
                if (HpRabcAdv::is_administrator(null, $this->external)) return true;

                // 下级用户才能更新
                $child_ids = HpRabcAdv::getUserAllChildren(null, $this->external);
            ...
            [['mobile'], 'mobile'],
            [['role_ids', 'depart_ids'], 'array', 'type' => 'string'],
            [['role_ids'], 'customer', 'callback' => function ($value) {
                if (HpRabcAdv::is_administrator(null, $this->external)) return true;

                list($role_ids, $children_ids) = HpRabcAdv::getUserRelatedAllRoles(null, $this->external);
                if (empty(array_diff($value, array_merge($role_ids,$children_ids)))) {
                    return true;
                }
                return '角色超出授权范围';
            }],
            [['depart_ids'], 'customer', 'callback' => function ($value) {
                if (HpRabcAdv::is_administrator(null, $this->external)) return true;
    ...
    public function run()
    {
        ...
        try {
            ...
            // 更新部门
            $this->updateDepart($db);

            // 清空缓存
            RabcRedis::clearRedis();
            HpRabcAdv::delRedisUserInfo($this->id);
    ...
    protected function updateDepart($db)
    {
        ...
        if (empty($this->depart_ids)) return;

        foreach ($this->depart_ids as $did) {
            $db->insert($this->external->relUserDepartTableName())
                ->cols([
                    'user_id' => $this->id,
                    'depart_id' => $did,
                    'del_time' => 0
